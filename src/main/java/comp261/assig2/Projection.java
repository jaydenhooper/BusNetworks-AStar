...
public class Projection {
     ...
     */
    public static Point2D model2Screen(GisPoint model, GraphController controller) {
        return new Point2D(
                model2ScreenX(model.getLon(), controller.getMapCanvas(), controller.getScale(),
                        controller.getRatioLatLon(), controller.getOrigin().getLon()),
                model2ScreenY(model.getLat(), controller.getMapCanvas(), controller.getScale(),
     ...
     */
    public static GisPoint Screen2Model(Point2D screenPoint, GraphController controller) {
        return new GisPoint(
    ...
    public static Point2D model2Screen(GisPoint model, Canvas mapCanvas, double scale, double ratioLatLon,
            GisPoint origin) {
    ...
    public static double model2ScreenX(double modelX, Canvas mapCanvas, double scale, double ratioLatLon,
            double originX) {
        return (modelX - originX) * (scale * ratioLatLon) + mapCanvas.getWidth() / 2;
    }

    public static double model2ScreenY(double modelY, Canvas mapCanvas, double scale, double originY) {
    ...
    public static GisPoint Screen2Model(Point2D screenPoint, Canvas mapcanvas, double scale, double ratioLatLon,
            GisPoint origin) {
    ...
    public static double getScreen2ModelX(Point2D screenPoint, Canvas mapcanvas, double scale, double ratioLatLon,
            double originX) {
        return ((screenPoint.getX() - mapcanvas.getWidth() / 2) / (scale * ratioLatLon)) + originX;
    }

    
    public static double getScreen2ModelY(Point2D screenPoint, Canvas mapCanvas, double scale, double originY) {
        // remember that y is flipped
